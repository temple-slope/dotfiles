...
```bash
# シークレット候補をスキャン
git diff HEAD | grep -iE '(api[_-]?key|api[_-]?secret|access[_-]?token|secret[_-]?key|password\s*=|private[_-]?key|-----BEGIN|AWS_|GITHUB_TOKEN|OPENAI_API)'
```
...
1. 問題のファイルと行番号を報告する
2. 修正方法を提案する（環境変数への移行、ファイルの .gitignore 追加など）
3. ユーザーが修正を完了したら Step 1 から再実行する

シークレットが既にコミット済みの場合:
- `git filter-repo` による履歴浄化をユーザーに依頼する（Claude は実行しない）
- 該当シークレットの無効化・ローテーションをユーザーに促す
...
各グループについて:
1. 関連ファイルのみをステージング（**`git add -A` 禁止**。必ず `git add path/to/file` でファイルを指定する）
